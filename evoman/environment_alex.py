...
class Environment(object):
    ...
    def checks_params(self):
        ...
        if self.goal not in ('kill','die'):
            self.print_logs("ERROR: 'goal' value must be 'kill' or 'die'.")
            sys.exit(0)
    ...
    def inverse_fitness(self):
        return 0.8*(100 - self.get_playerlife()) * 0.2*self.get_enemylife() - numpy.log(self.get_time())
        # custom fitness function to make the agent kill himself as fast as possible
